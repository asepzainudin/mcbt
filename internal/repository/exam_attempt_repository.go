...
func (r *ExamAttemptRepository) ListCandidateExams(ctx context.Context, studentID uuid.UUID) ([]CandidateExamRow, error) {
	...
	args := []interface{}{studentID, studentID, studentID, studentID, studentID, studentID, studentID, studentID}
	err := r.db.WithContext(ctx).
		Table("exams e").
		Select(`
			e.id            AS exam_id,
			...
			e.allow_discussion AS allow_discussion,
			...
			EXISTS (SELECT 1
			  ...
			  WHERE a.exam_id = e.id AND a.student_id = ?
			    AND a.status = 'submitted'
			    AND q.question_type = 'essay'
			    AND ea.graded_at IS NULL) AS essay_ungraded,
			EXISTS (SELECT 1 FROM exam_participants ep2
			  WHERE ep2.exam_id = e.id AND ep2.student_id = ?) AS is_assigned,
			(SELECT count(*) FROM exam_attempts ea
			  WHERE ea.exam_id = e.id AND ea.student_id = ?) AS attempts_used,
			(SELECT ea2.id FROM exam_attempts ea2
			  WHERE ea2.exam_id = e.id AND ea2.student_id = ?
		...
		Where("e.status = ?", model.ExamStatusPublished).
